...
export function tokenize(sourceCode:string,debugMode:boolean):Token[] {
    ...
    while (src.length>0) {
        if (src[0]=="(") {
            ...
            let str = ""
            while (src.length>0&&(isalpha(src[0])||isint(src[0]))) { // Do while current letter is alphanumeric
                str += src.shift()
            }
            if (keywords[str]) {
            ...
            let num = ""
            while (src.length>0&&isint(src[0])) { // Do while current letter is alphanumeric
                num += src.shift()
            }
            ...
            throw ("invalid token, here's what i got:"+src[0]+src[0].charCodeAt(0))
        }
    }
